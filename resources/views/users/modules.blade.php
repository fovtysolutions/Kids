@php
    ...
    $admin_settings = json_encode(getAdminAllSetting());
@endphp
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(".total").text(formatCurrency(total,'{{ $admin_settings }}'));
        });
    ...
    <script>
        function ChangePrice(user = null,workspace = null,user_module_price = 0 ) {
            ...
            $(".userpricetext").text(formatCurrency(userpricetext,'{{ $admin_settings }}'));
            $(".workspacepricetext").text(formatCurrency(workspacepricetext,'{{ $admin_settings }}'));
            $(".userprice_input").val(formatCurrency(userpricetext,'{{ $admin_settings }}'));
            $(".workspaceprice_input").val(formatCurrency(workspacepricetext,'{{ $admin_settings }}'));
            Coupon();
        ...
        function ChangeModulePrice() {
                    ...
                    $("#extension_div").append(`<div class="col-md-6 col-sm-6  my-2">
                                    <div class="d-flex align-items-start">
                                        ...
                                        <div class="ms-2">
                                            ...
                                            <h4 class="mb-0 text-primary">` + formatCurrency(price,'{{ $admin_settings }}') + `<span class="text-sm">`+time+`</span></h4>
                                        </div>
                                    </div>
                                </div>`);
            ...
            $(".module_price_text").text(formatCurrency(user_module_price,'{{ $admin_settings }}'));
    ...
    <script>
        ...
        function SwitchChange()
        {
            ...
            $(".plan-price-text").text(formatCurrency(planprice,'{{ $admin_settings }}'));
            $(".plan-time-text").text(time);

            $(".planpricetext").html('<span class="final_price">'+ formatCurrency(planprice,'{{ $admin_settings }}')  + '</span>');
            $(".user-price").text('( {{ _("Per User") }}'+ formatCurrency(userprice,'{{ $admin_settings }}') +')');
            $(".userpricetext").text(formatCurrency(userpricetext,'{{ $admin_settings }}'));
            $(".workspace-price").text('( {{ __("Per Workspace")}} '+ formatCurrency(workspaceprice,'{{ $admin_settings }}') +')');
            $(".workspacepricetext").text(formatCurrency(workspacepricetext,'{{ $admin_settings }}'));
    ...
    <script>
        ...
        function Coupon()
        {
            ...
            $(".total").text(formatCurrency(fp,'{{ $admin_settings }}'));
        }
    </script>
