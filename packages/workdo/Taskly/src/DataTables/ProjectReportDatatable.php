...
class ProjectReportDatatable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('status',function(Project $project){
            if ($project->status == 'Finished') {
                $html= '<span class="badge rounded-pill p-2 px-3  bg-success">' .'Finished' . '</span>';
            }
            elseif($project->status == 'Ongoing') {
                $html= '<span class="badge rounded-pill p-2 px-3  bg-secondary">' . 'Ongoing' . '</span>';
            }
            else {
                $html= '<span class="badge rounded-pill p-2 px-3  bg-warning">' . 'OnHold'. '</span>';
            }
            return $html;
        });
    ...
    {
        $projects = $model->select('projects.*')->where('projects.created_by', creatorId())
        ->projectOnly()
        ->where('projects.workspace', getActiveWorkSpace())
        ->groupBy('projects.id');

        if (!empty($request->user)) {

            $projects = $projects->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->projectonly()->where('user_projects.user_id', '=', $request->user);
        }
