...
@section('page-breadcrumb')
    {{ __('Project Details') }},{{ __("Bug Report") }}
@endsection
@push('css')
...
<div class="d-flex">
    @stack('addButtonHook')
    <a href="{{ route('projectbug.list',[$project->id]) }}" class="btn btn-sm btn-primary me-2" data-bs-toggle="tooltip"title="{{ __('List View') }}">
        <i class="ti ti-list text-white"></i>
    </a>
    @permission('bug create')
    <a class="btn btn-sm btn-primary me-2" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Bug') }}" data-url="{{route('projects.bug.report.create',$project->id)}}" data-toggle="tooltip" title="{{ __('Add Bug') }}"><i class="ti ti-plus"></i></a>
    @endpermission
    <a href="{{route('projects.show',[$project->id])}}" class="btn-submit btn btn-sm btn-primary" data-toggle="tooltip" title="{{ __('Back') }}"><i class="ti ti-arrow-back-up"></i></a>
</div>
                            ...
                            <div id="{{'task-list-'.str_replace(' ','_',$stage->id)}}" data-status="{{$stage->id}}" class="card-body kanban-box">
                                ...
                                <div class="card" id="{{$bug->id}}">
                                    <div class="position-absolute top-0 start-0 pt-3 ps-3">
                                      @if($bug->priority =='Low')
                                            <div class="badge bg-success p-2 px-3"> {{ $bug->priority }}</div>
                                        @elseif($bug->priority =='Medium')
                                            <div class="badge bg-warning p-2 px-3"> {{ $bug->priority }}</div>
                                        @elseif($bug->priority =='High')
                                            <div class="badge bg-danger p-2 px-3"> {{ $bug->priority }}</div>
                                        @endif
                                    </div>
                                        ...
                                        <div class="card-header-right">
                                            <div class="btn-group card-option">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    @permission('bug show')
                                                    <a class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('View Bug') }}" data-url="{{route('projects.bug.report.show',[$bug->project_id,$bug->id])}}"><i class="ti ti-eye me-1"></i>
                                                        {{__('View')}}</a>
                                                    @endpermission
                                                    @permission('bug edit')
                                                    <a class="dropdown-item" data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Bug') }}" data-url="{{route('projects.bug.report.edit',[$bug->project_id,$bug->id])}}"><i class="ti ti-pencil me-1"></i>
                                                        {{__('Edit')}}</a>
                                                    @endpermission
                                                    ...
                                                    <form id="delete-form-{{$bug->id}}" action="{{ route('projects.bug.report.destroy',[$bug->project_id,$bug->id]) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a href="#" class="dropdown-item bs-pass-para show_confirm" data-confirm="{{__('Are You Sure?')}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}"  data-confirm-yes="delete-form-{{$bug->id}}"><i class="ti ti-trash me-1"></i>
                                                            {{__('Delete')}}
                                                        </a>
                                                    </form>
                                                    @endpermission
        ...
        <script>
            $(document).on('click', '#form-comment button', function (e) {
                ...
                if (comment != '') {
                    ...
                    $('.comment-requried').text('Please write comment!')
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.deleteUrl.length > 0) {
                            delLink = "<a href='#' class='delete-icon delete-comment-file action-btn btn-danger  btn btn-sm d-inline-flex align-items-center '  data-url='" + data.deleteUrl + "'>" +
                                "                                        <i class='ti ti-trash'></i>" +
                                "                                    </a>";
                        }
