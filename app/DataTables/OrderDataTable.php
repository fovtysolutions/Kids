...
class OrderDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('payment_status', function (Order $order) {
                ...
                {
                    $html = '<span class="bg-success p-1 px-3 text-white">'. ucfirst($order->payment_status) .'</span>';

                }
                else{
                    $html = '<span class="bg-danger p-2 px-3 text-white">'. ucfirst($order->payment_status) .'</span>';
                }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('coupon_code')->title(__('Coupon')),
